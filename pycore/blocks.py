...
def block_ConvBnRelu6( name, botton, top, s_filer=256, n_filer=64, offset="(1,0,0)", size=(32,32,3.5), opacity=0.5 ):
    return [
    to_Conv(
        name="ccr_{}".format( name ),
        s_filer=str(s_filer),
...
def inv_block(num, name, bottom, top, s_filer=256, n_filer=64, offset="(0,0,0)", size=(32, 32, 3.5)):
    lys = []
    layers = [*['{}_{}'.format(name, i) for i in range(num - 1)], top]
    for name in layers:
        ly = [to_Conv(
